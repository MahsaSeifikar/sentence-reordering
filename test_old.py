...
def main_second_sentence_v3(tokenizer, second_model, max_seq_length, test):
    # find fist sentence
    cou += 1
    best_comb = 0
    best_score = -10000
...
def main():
    ...
    for test in test_list:
        pool.apply_async(main_second_sentence_v3, args=(tokenizer, second_model, max_seq_length, test, ), callback=get_result)
        cou += 1
        print(cou)
    pool.close()
    pool.join()
